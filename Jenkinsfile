  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --package-branch tsfc 0d-mesh --package-branch fiat vertexcell --package-branch ufl vertexcell --package-branch petsc4py fix-return-type --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --no-package-manager || (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }
